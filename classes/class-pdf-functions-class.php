        ...
        class WC_pdf_functions {
			...
			public static function woocommerce_completed_order_create_invoice( $order_id ) {
				...
				if ( !get_post_meta( $order_id, '_invoice_number', TRUE ) ) {
					...
					$_invoice_number_display = WC_pdf_functions::create_display_invoice_number( $next_invoice, $order_id );

					update_post_meta( $order_id, '_invoice_number', $next_invoice );
					update_post_meta( $order_id, '_invoice_number_display', $_invoice_number_display );
					...
					$invoice_meta = array( 
							...
							'pdf_company_name' 			=> get_post_meta( $order_id, '_pdf_company_name', TRUE ),
							'pdf_company_information' 	=> get_post_meta( $order_id, '_pdf_company_information', TRUE ),
							'pdf_registered_name' 		=> get_post_meta( $order_id, '_pdf_registered_name', TRUE ),
							'pdf_registered_office' 	=> get_post_meta( $order_id, '_pdf_registered_office', TRUE ),
					...
					// Add order note
					$order->add_order_note( sprintf( __("Invoice number %s created", 'woocommerce-pdf-invoice' ), $_invoice_number_display ), false, false );
			...
			public static function set_invoice_date( $order_id ) {
				...
				if ( get_post_meta($order_id, '_invoice_date', TRUE) && get_post_meta($order_id, '_invoice_date', TRUE) != '' ) {
					...
					if ( $woocommerce_pdf_invoice_options['pdf_date'] == 'completed' ) {
						...
						update_post_meta( $order_id, '_invoice_date', wc_format_datetime( $date ) );
			 ...
			 */
			static function create_display_invoice_number( $next_invoice, $id ) { 
